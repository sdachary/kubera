#!/usr/bin/env node
// Kubera design tokens build.
// Reads design/tokens/kubera.tokens.json (W3C DTCG-flavored), emits one Tailwind v4 CSS file.
...
const TOKENS = resolve(ROOT, "design/tokens/kubera.tokens.json");
const OUT = resolve(ROOT, "app/assets/tailwind/kubera-design-system/_generated.css");

const HEADER = `/*
 * GENERATED — do not edit by hand.
 * Source: design/tokens/kubera.tokens.json
...
function assertKnownRef(ref, source) {
  if (VALID_PATHS && !VALID_PATHS.has(ref)) {
    throw new Error(
      ...
      `Add it to design/tokens/kubera.tokens.json or fix the typo.`
    );
  }
}
...
function build() {
  ...
  for (const [path, node] of walk(tokens)) {
    if (path[0] === "utility") {
      ...
      if (ext["kubera.compose"]) {
        utilityBlocks.push(`@utility ${name} {\n  @apply ${ext["kubera.compose"].join(" ")};\n}`);
        continue;
      }

      const prefix = ext["kubera.utility"]?.prefix;
      const raw = ext["kubera.utility"]?.raw;
      const dark = ext["kubera.dark"];
    ...
    const dark = node.$extensions?.["kubera.dark"];
    if (dark !== undefined) {
